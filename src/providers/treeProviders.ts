...
import { getBrainService, inferDomain, type Doc } from "../services/brainService";
...
export class CategoryTreeProvider implements vscode.TreeDataProvider<BrainTreeItem> {
  ...
  getChildren(element?: BrainTreeItem): Thenable<BrainTreeItem[]> {
    ...
    const getEffectiveDomain = (doc: Doc): string => {
...
export class StatsTreeProvider implements vscode.TreeDataProvider<BrainTreeItem> {
  ...
  getChildren(element?: BrainTreeItem): Thenable<BrainTreeItem[]> {
    ...
    const dbPathItem = new BrainTreeItem("Chemin DB", vscode.TreeItemCollapsibleState.None);
    ...
    const lastDocMs = Math.max(0, ...service.getAllDocs().map((d) => toMs(d.created_at)));
    const lastPitfallMs = Math.max(0, ...service.getAllPitfalls().map((p) => toMs(p.created_at)));
    ...
    lastActivityItem.description = lastMs > 0 ? new Date(lastMs).toLocaleString("fr-FR") : "Jamais";
